...
def calculate_bottom_indicators(df):
    ...
    close_above_ma10 = C > MA10*0.98; is_yang_line = C > O; vol_normal = volume_ratio < 2.5
    ...
    return {
        'close':float(C[-1]),'cn_score':float(cn_score[-1]),'us_score':float(us_score[-1]),
...
def calculate_strong_indicators(df):
    ...
    return {
        'close':float(C[-1]),'trend_score':float(trend_score[-1]),'volume_ratio':float(volume_ratio[-1]),
...
class FutuClient:
    ...
    def _batch_kline(self, codes, num=200):
        ...
        for i in range(0, total, bs):
            ...
            if ret != RET_OK: logger.warning(f"Batch {bn} subscribe failed: {msg}"); continue
            time.sleep(2.5)
            for code in batch:
                try:
    ...
    def scan_market_all(self, market, max_stocks=500, min_volume=50000):
        """
        全市场4策略扫描
        ...
        logger.info(f"After snapshot filter: {len(valid)}")

        if not valid: return {'error': '无符合条件的股票'}

        scan_codes = valid[:max_stocks]
        logger.info(f"Scanning top {len(scan_codes)} stocks")

        day_data = self._batch_kline(scan_codes)
        ...
        # 2. MACD底背离
        div = []
        for code, df in list(day_data.items())[:200]:
            try:
        ...
        return {
            ...
            'scanned': len(scan_codes), 'kline_obtained': len(day_data),
            'pullback_ma5_ma10': {'count': len(pb), 'results': pb[:30]},
            'macd_divergence': {'count': len(div), 'results': div[:30]},
            'bottom_screening': {'count': len(bot), 'results': bot[:20]},
            'strong_momentum': {'count': len(strong), 'results': strong[:20]},
        }